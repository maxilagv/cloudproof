# Prompt para Codex — Subfase 1.D (`cloudproof reproduce` en vivo)
...
1. docs/PHASE_1.md (sección "Subfase 1.D — cloudproof reproduce en vivo")
2. packages/cli/src/commands/reproduce.ts (estado actual: findAssertion()
   ya lee bundles guardados en .cloudproof/*.json y ubica la assertion fallida;
...
4. packages/postgres-verifier/src/verify.ts (para confirmar qué estado
   ...
   CloudProof Bundle salen de una única combinación — la imagen A0 (buildeada
   ...
   resolver ahora)
5. packages/cloudproof-schema/src/cloudproof-bundle.ts (el contrato de CloudProofBundle/
   Assertion tal como existe hoy)
6. packages/cli/src/commands/release-verify.ts (cómo se carga
   cloudproof.config.ts con @cloudproof/config para obtener el servicePath, y cómo
...
servicePath (viene de cloudproof.config.ts vía @cloudproof/config). Decidí cómo
...
CloudProofBundleSchema actual ya lo tiene — evaluá si alcanza con eso antes de
tocar el schema. Si concluís que hace falta un campo nuevo en Assertion o
en el CloudProof Bundle para que esto funcione bien, hacelo ADITIVO (un campo
...
1. cómo resolviste el problema de contexto faltante (tocaste el schema o
   alcanzó con lo que ya expone CloudProofBundle) y por qué
2. archivos tocados
...
problema de contexto faltante hace falta tocar packages/cloudproof-schema,
...
docs/PHASE_1.md: "cloudproof reproduce <id> deja al usuario con una app
...
CLOUDPROOF_DOCKER_IT=1, y si tu test tarda por pulls de imágenes usá el mismo
criterio de CLOUDPROOF_DOCKER_IT_FULL=1 si aplica):
...
2. Corré el flujo de verify (verifyRelease o runReleaseVerify) contra ese
   repo para obtener un CloudProof Bundle real con al menos una assertion
   result:"fail", y guardalo en .cloudproof/ como ya hace release-verify.ts.
...
4. Confirmá que la app quedó corriendo y responde en la URL impresa
   ...
   que usa docker-executor: "docker ps -aq --filter label=dev.cloudproof.run=<runId>"
   vacío después de limpiar).
Corré también "pnpm --filter @cloudproof/cli typecheck" y confirmá que el
...
packages/cloudproof-schema SOLO si concluiste que hace falta un campo aditivo
...
ejemplo, el servicePath cuando hay más de un servicio en cloudproof.config.ts),
